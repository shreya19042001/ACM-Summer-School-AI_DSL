...
gen_prologue()
{       
        genf <<"#include <iostream>\n";
        genf <<"#include \"api.h\"\n"; //Tensor class
        genf <<"using namespace std;\n";
	genf <<"int main()\n{\n";
}
...
gen_decl(string name, Type_Info *type_info)
{
    ...
    {
        // Generate tensor declaration
        // Tensor<base_type> id_name(dim, row, col);
        ...
        int dim = type_info->get_dim();
        int row = type_info->get_row();
        int col = type_info->get_col();
