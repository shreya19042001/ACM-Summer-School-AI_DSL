...
int Type_Info::get_dim() const {
    return dim;
}

int Type_Info::get_row() const {
    return row;
}

int Type_Info::get_col() const {
    return col;
}
...
Type_Info *
type_check_conv(Ast *left, Ast *right)
{
	Type_Info *left_type_info = left->get_type_info();
	assert (left_type_info);
	...
	if (!left_type_info->is_tensor_type()
	    || !right_type_info->is_tensor_type())
	{
		cerr << "Error: Operands for conv should have tensor type\n";
		exit (1);
	}
	...
	if (left_type_info->get_number_of_dimensions() != 2
	    || right_type_info->get_number_of_dimensions() != 2)
	{
		cerr << "Error: Tensor operands for conv should have 2 dimensions\n";
		exit (1);
	}
